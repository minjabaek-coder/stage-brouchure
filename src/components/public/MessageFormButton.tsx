...
/**
 * Pill-shaped button that opens a native <dialog> with the message form.
 * On successful submit, calls router.refresh() so server-rendered lists
 * (home preview + /messages page) pick up the new entry.
 */
const MessageFormButton: FC<MessageFormButtonProps> = ({
  ...
  const handleSubmit = async (e: FormEvent<HTMLFormElement>) => {
    ...
    if (trimmedBody.length < 1 || trimmedBody.length > BODY_MAX) {
      toast.error(`메시지는 1–${BODY_MAX}자여야 합니다.`);
      return;
    }
    setSubmitting(true);
    try {
      ...
      if (res.status === 429) {
        toast.error("잠시 후 다시 시도해 주세요. (1분에 1회만 작성 가능)");
        setSubmitting(false);
        return;
      }
      if (!res.ok) {
        const err = (await res.json().catch(() => ({}))) as ApiError;
        toast.error(err.error?.message ?? "메시지를 남기지 못했어요.");
        setSubmitting(false);
        return;
      }
      toast.success("메시지를 남겼습니다.");
      handleClose();
      router.refresh();
    } catch {
      ...
      >
        <div className="relative px-6 pt-7 pb-6">
          <button
            ...
            data-testid="message-dialog-close"
          >
          ...
          <form onSubmit={handleSubmit} className="mt-5 flex flex-col gap-3">
            <label className="flex flex-col gap-1.5">
              ...
              <input
                ...
                style={{ borderWidth: "0.5px" }}
                data-testid="message-input-nickname"
              />
            </label>

            <label className="flex flex-col gap-1.5">
              <span className="text-muted text-[12px] tracking-[-0.01em]">
                메시지 ({body.length}/{BODY_MAX})
              </span>
              <textarea
                value={body}
